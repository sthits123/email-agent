...
def main():
    ...
    try:
        ...
        # Run the OAuth flow
        print("\nNote: If you are on a remote server, you might need to use the 'Out-of-band' flow.")
        try:
            # Try local server first
            credentials = flow.run_local_server(port=0, open_browser=False)
        except Exception:
            # Fallback to a simpler flow or provide instructions for OOB
            print("Local server failed. Trying to provide a manual authorization URL...")
            # For modern Google libraries, run_local_server with open_browser=False
            # is the way to go, and it prints the URL to the console.
            credentials = flow.run_local_server(port=0, open_browser=False)
